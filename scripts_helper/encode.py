...
def main():
    ...
    parser.add_argument('--annotations', type=str, default='dataset/csvfiles/annotations.csv', help='annotation file')
    parser.add_argument('--class_mapping', type=str, default='dataset/csvfiles/class_name_mapping.csv', help='class mapping file')
    ...
    # MARK: add label to feature file
    meta_data = pd.read_csv(args.annotations)
    class_csv = pd.read_csv(args.class_mapping)
    class_num = class_csv.shape[0]
    print(f"There are {class_num} classes in total.")

    labels = meta_data['label'].unique()
    labels.sort()
    print(f"There are {len(labels)} labels in total.")
    new_class_num = len(labels)
    new_lookup_table = {}
    for i, label in enumerate(labels):
        new_lookup_table[label] = i
    
    prepared_data = []
    for _, item in tqdm(meta_data.iterrows(), total=len(meta_data)):
        sample_id = item['sample_id']
        onset = item['onset']
        offset = item['offset']
        category = item['label']
        # 转换成新的类别
        new_category = new_lookup_table[category]
         # 跳过没有特征的
        if not os.path.exists(os.path.join(args.feature_path, str(sample_id)+".pkl")):
            continue

        with open(os.path.join(args.feature_path, str(sample_id)+".pkl"), "rb") as f:
            fea = pickle.load(f)
        if fea['video_features'].shape[0] < 10 or fea['audio_features'].shape[0] < 10:
            continue
        # 特征维度和模型输入维度不匹配，需要调整
        if fea['video_features'].shape != (10, 7, 7, 512):
            fea['video_features'] = fea['video_features'].transpose(0, 3, 1, 2)
        
        # 还有其他有问题的，单独拿出来
        if fea['video_features'].shape != (10, 7, 7, 512):
            fea['video_features'] = fea['video_features'].transpose(0, 3, 1, 2)
            with open(output_log, "a") as f:
                f.write(str(sample_id)+ " video features shape error, [{}]".format(fea['video_features'].shape) + '\n')
            continue
        if fea['audio_features'].shape != (10, 128):
            with open(output_log, "a") as f:
                f.write(str(sample_id)+ " audio features shape error, [{}]".format(fea['audio_features'].shape) + '\n')

        onset_second = round(onset)
        offset_second = round(offset)
        # 用的是每秒的one-hot标签
        onehot_labels = np.zeros((10, new_class_num))
        for i in range(onset_second, offset_second):
            onehot_labels[i][new_category] = 1
        fea['onehot_labels'] = onehot_labels
        with open(os.path.join(feature_path, str(sample_id)+".pkl"), 'wb') as f:
            pickle.dump(fea, f)
        # prepared_data.append(sample_id)
