...
class VisualAdapter(nn.Module):
	...
	def forward(self, x, vis_token=None):
		if self.adapter_kind == "bottleneck" and self.is_multimodal:
			...
			att_v2tk = torch.bmm(rep_token, vis_token.squeeze(-1))  # 注意力矩阵，计算视觉token和latent_token的注意力分数

			att_v2tk = F.softmax(att_v2tk, dim=-1)
			# att_v2tk 作为权重应用于 vis_token，生成一个新的 rep_token_res
			rep_token_res = torch.bmm(att_v2tk, vis_token.squeeze(-1).permute(0,2,1))
			# 整合了视觉信息和原始 token 信息
			rep_token = rep_token + rep_token_res
			

			# 计算rep_token和输入x之间的注意力分数
			...
			# 将 x_res 根据一个门控系数 self.gate_av 加权，并与原始输入 x 相加。这步操作是为了将视觉 token 与输入 x 中的特征结合
